  ...
  return (
    <div className="bg-surface-container-lowest rounded-xl shadow-sm flex flex-col overflow-hidden">
      <div className="w-full overflow-x-auto">
        <table className="w-full text-left font-body-md font-body-md text-on-surface">
          <thead>
            ...
            {staff.map((s, idx) => {
              ...
              return (
                <tr key={s.id} className={`${idx % 2 === 1 ? 'bg-surface-container-low' : 'bg-surface-container-lowest'} hover:bg-surface-container-low transition-colors`}>
                  <td className="py-3.5 px-4 text-center font-label-md font-label-md text-on-surface-variant">{s.id}</td>
                  <td className="py-3.5 px-4 font-mono font-body-sm font-body-sm text-on-surface">{s.niy}</td>
                  <td className="py-3.5 px-4">
                    <div className="flex items-center gap-space-xs">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center font-headline-sm flex-shrink-0 ${avatarClasses[idx]}`}>
                        {initialsOf(s.name)}
                      </div>
                      <div className="flex flex-col min-w-0">
                        <span className="font-body-md-medium font-body-md-medium text-on-surface truncate">{s.name}</span>
                        <span className="font-label-sm font-label-sm text-on-surface-variant">{displayRole}</span>
                      </div>
                    </div>
                  </td>
                  <td className="py-3.5 px-4">
                    <a className="inline-flex items-center gap-1 font-body-sm font-body-sm-medium text-body-sm-medium text-secondary hover:underline" href="#">
                  ...
                  <td className="py-3.5 px-4 text-center">
                    {s.status === 'Aktif' ? (
                      <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full font-body-sm font-body-sm-medium text-body-sm-medium bg-emerald-50 text-emerald-600">
                        <span className="w-1.5 h-1.5 rounded-full bg-emerald-600"></span>
                        Aktif
                      </span>
                    ) : (
                      <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full font-body-sm font-body-sm-medium text-body-sm-medium bg-rose-50 text-rose-700">
                  ...
                  <td className="py-3.5 px-4 text-right pr-6">
                    <div className="inline-flex items-center justify-end gap-1.5">
                      <button className="h-8 px-2.5 rounded-lg bg-blue-50 hover:bg-blue-100 text-secondary font-label-md font-label-md flex items-center gap-1 transition-colors" title="Detail Pegawai" type="button">
                        <span className="material-symbols-outlined text-[16px]">visibility</span>
                        <span>Detail</span>
                      </button>
                      <button className="h-8 px-2 rounded-lg bg-amber-50 hover:bg-amber-100 text-amber-700 font-label-md font-label-md flex items-center gap-1 transition-colors" title="Reset Sandi Presensi" type="button">
                      ...
                      {s.status === 'Aktif' ? (
                        ...
                        <button className="h-8 px-2.5 rounded-lg bg-emerald-50 hover:bg-emerald-100 text-emerald-600 font-label-md font-label-md flex items-center gap-1 transition-colors" title="Aktifkan Kembali" type="button">
      ...
      <div className="p-space-md bg-surface-container-lowest flex flex-col sm:flex-row items-center justify-between gap-space-sm">
        <div className="flex items-center gap-space-xs text-on-surface-variant font-body-sm font-body-sm">
          <span>Menampilkan <strong className="font-body-sm-medium text-on-surface">1–{staff.length}</strong> dari <strong className="font-body-sm-medium text-on-surface">{state.staff.length}</strong> data pegawai</span>
