...
function EditEmployeeModal({ staff, onClose, onSave }) {
  ...
  const [niy, setNiy] = useState(staff?.niy || '')
  const [name, setName] = useState(staff?.name || '')
  const [unitId, setUnitId] = useState(staff?.unitId || (state.units.length > 0 ? state.units[0].id : ''))
  const [jabatan, setJabatan] = useState(staff?.role || '')
  const [status, setStatus] = useState(staff?.status || 'Aktif')
...
const updatedStaff = {
       ...
       unitId,
       status,
       // Preserve existing attendance fields
       masuk: staff.masuk,
       method: staff.method,
       late: staff.late,
       alpha: staff.alpha,
        ...
        <div className="px-space-lg py-space-md border-b border-outline/20 flex items-start justify-between bg-surface-container/50">
          <div>
            ...
            <p className="font-body-sm text-body-sm text-on-surface-variant mt-0.5" id="modalSubtitle">
              Ubah data pegawai yang dipilih.
            </p>
          </div>
          <button
            onClick={onClose}
