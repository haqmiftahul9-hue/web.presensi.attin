...
function UnitCards() {
  ...
  // Kartu unit di-derive dari satu sumber data (store): nama unit, icon, dan jumlah pegawai.
  const getSubtitle = (unitId) => {
    switch (unitId) {
      case 'tk': return '100% Terverifikasi Presensi'
  ...
  const unitData = state.units.map((u) => ({
    icon: u.icon,
    label: u.nama,
    count: `${state.staff.filter((s) => s.unitId === u.id).length} Pendidik/Staf`,
    subtitle: getSubtitle(u.id),
  }))
