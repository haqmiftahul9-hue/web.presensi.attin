...
export function selectRekapTableData(state) {
  ...
  return attendance.map((a) => {
    ...
    const unitPulang = unit ? unit.pulang : '15:00'
    const isHadir = a.masuk !== null
    let stMasuk = 'Alpha'
    ...
    if (isHadir) {
      stMasuk = a.late > 0 ? `Terlambat ${a.late} menit` : 'Tepat Waktu'
      stMasukType = a.late > 0 ? 'late' : 'good'
      stPulang = 'Tepat Waktu'
      stPulangType = 'good'
      ket = a.unitId === 'smp' ? 'Tugas Pengawas Ujian' : a.unitId === 'sd' ? (a.id === 2 ? 'Dispensasi Rapat Gugus' : a.id === 3 ? 'Lupa tap pulang / verifikasi TU' : 'Guru Kelas Reguler') : a.unitId === 'sma' ? (a.id === 6 ? 'Cuti Alasan Penting (Lampiran SK)' : 'Guru Kelas Reguler') : a.unitId === 'tk' ? 'Izin dinas luar jam 14:30' : 'Surat Sakit Dokter RS Radja'
    } else if (a.status === 'Tidak Aktif') {
      stMasuk = 'Izin'
      stMasukType = 'leave'
      stPulang = 'Izin'
      stPulangType = 'leave'
    ...
    return {
      ...
      niy: a.niy,
      unit: unitName,
      masuk: isHadir ? `${a.masuk} WIB` : '-',
      pulang: isHadir ? `${unitPulang} WIB` : '-',
      stMasuk,
      stMasukType,
      stPulang,
      stPulangType,
